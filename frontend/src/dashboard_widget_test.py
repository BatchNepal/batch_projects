...
with sync_playwright() as p:
    ...
    page.goto(f"{base}/workspace/dashboards", wait_until="networkidle", timeout=30000)
    page.wait_for_timeout(800)
    page.get_by_text("Blank report", exact=True).click()
    page.wait_for_timeout(1500)
    page.screenshot(path="/home/frappe/batcherp/apps/batch_projects/frontend/src/dash1.png")
    print("URL:", page.url)
    print("errors:", [e for e in errors if "session/bootstrap" not in e])
    # dump visible button/text content to find "add widget" affordance
    print(page.locator('button').all_inner_texts()[:40])
    browser.close()
