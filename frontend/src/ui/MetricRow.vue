<template>
  <div class="flex items-start flex-wrap gap-12">
    <div v-for="metric in metrics" :key="metric.label" class="flex flex-col min-w-[72px]">
      <span class="text-xs font-medium uppercase tracking-wider text-muted leading-none mb-2">
        {{ metric.label }}
      </span>
      <span
        class="text-metric font-semibold leading-none tabular-nums"
      ...
      <span v-else-if="metric.sub" class="text-xs text-muted mt-2 leading-none">
        {{ metric.sub }}
      </span>
    </div>
  </div>
